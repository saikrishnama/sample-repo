...
class XRayTrace:
    ...
    def __init__(self, trace_id: str, xray_client: Optional[Any] = None):
        """
    ...
    def _retrieve_trace(self) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If the trace is missing or empty.
        """
        try:
            ...
            if not traces:
                raise ValueError(f"No trace found for ID: {self.trace_id}")
            ...
            logger.error("Failed to retrieve X-Ray trace: %s", exc)
            raise
    ...
    def retrieve_id(self, step_name: str) -> Optional[str]:
        ...
        if not stepfunctions_segment:
            logger.warning("StepFunctions segment not found")
            return None
        ...
        if not step_subsegment:
            logger.warning("Step subsegment not found for step: %s", step_name)
            return None
